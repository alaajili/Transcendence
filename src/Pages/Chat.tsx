...
const Chat = () => {
    ...
    async function getdminfos(id: number) {
        let room: intersetchannel = {name: "", img: "", id: 0, status: "dm"}
        try {
            const res = await axios.get(
                ...
                if (msgs[i].senderId == id) {
                    ...
                    msg = [
                        ...msg,
                        {
                            message: msgs[i].content,
                            isSentByMe: false,
                            img: "http://localhost:3000/" + msgs[i].senderId + ".png",
                        },
                    ];
                }
            }
    ...
    useEffect(() => {
        ...
        socket?.on("newmessage", async (dto: messagedto) => {
            setMessages((prevMessages) => [
                ...prevMessages,
                {
                    message: dto.message,
            ...
            {selectedChannel ? (
                <div className="child-container-2">
                    <div className="container-2 font-satoshi text-white w-[65vw] h-[90vh] max-sm:w-[80vw] max-sm:h-[45vh] max-md:w-[80vw] max-md:h-[45vh] flex flex-col justify-center items-start relative overflow-hidden">
                        ...
                        {!challengebutton ? (
                            <>
                                <button className="container-1 absolute top-[2.2vh] right-[2vw] px-[1.5vw] py-[.4vw] mr-[3vw] uppercase font-bold hover:scale-105 text-[.7vw]"
                                    onClick={leaveroom}
                                >
                                    leave channel
                                </button>
